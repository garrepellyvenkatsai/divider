        ...
        <body>
          ...
          <script>
            window.addEventListener('message', (event) => {
              const { xml, taskColor, eventColor, gatewayColor, isRough } = event.data;
    
              const bpmnModeler = new window.BpmnJS({ container: '#diagramContainer' });
              ...
              bpmnModeler.importXML(xml).then(() => {
                ...
                colorizeElements();
                if (isRough) {
                  applyRoughStyle(document.querySelector('svg'));
                }
              }).catch(err => {
